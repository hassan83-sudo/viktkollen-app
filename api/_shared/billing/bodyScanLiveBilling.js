...
export const BODY_SCAN_BILLING = Object.freeze({
  feature_id: 'body.scan',
  provider_id: 'openai',
  quantity: 1,
  route: 'api/body-analysis/index.js',
  unit: 'requests',
})

let testRuntimeOverride = null

export function setBodyScanBillingRuntimeForTests(runtime = null) {
  testRuntimeOverride = runtime
}

export function installBodyScanBillingTestRuntime(overrides = {}) {
  ...
  try {
    operationStore = createUsageBackedDispatchStore({
      recordUsage,
      usageEventPlan: bodyScanUsageEventPlan,
      usageRepository,
    })
  } catch {
  ...
  setBodyScanBillingRuntimeForTests(runtime)
  return runtime
}

export function bodyScanUsageEventPlan({ operationId, userId } = {}) {
  const identity = createBillingAccountingIdentity(operationId)
  return Object.freeze({
    cost_basis: 'UNAVAILABLE',
    event_id: identity?.event_id || operationId,
    event_type: BODY_SCAN_BILLING.feature_id,
    feature: BODY_SCAN_BILLING.feature_id,
    metadata: Object.freeze({
      usage_basis: 'UNAVAILABLE',
    }),
    provider: BODY_SCAN_BILLING.provider_id,
    quantity: BODY_SCAN_BILLING.quantity,
    reference_id: identity?.reservation_id || operationId,
    unit: BODY_SCAN_BILLING.unit,
    user_id: userId,
  })
}

export async function resolveBodyScanBillingRuntime() {
  ...
  try {
    ...
    const featureControl = await readControlRow(client, 'feature_controls', 'feature_id', BODY_SCAN_BILLING.feature_id)
    const providerRow = await readControlRow(client, 'provider_controls', 'provider_id', BODY_SCAN_BILLING.provider_id)
    const recordUsage = (input) => recordUsageEvent(input, usageRepository)
...
export function createBodyScanOperationId({ clientAttemptId, userId }) {
  return createScopedOperationId({
    clientAttemptId,
    featureId: BODY_SCAN_BILLING.feature_id,
    route: BODY_SCAN_BILLING.route,
    userId,
  })
}

export const mapBodyScanBillingHttp = mapFoodScanBillingHttp

export async function executeBodyScanMeteredOperation({
  executeProvider,
  instanceKey,
  log,
  operationId,
