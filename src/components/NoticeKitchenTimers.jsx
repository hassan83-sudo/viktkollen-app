...
function NoticeKitchenTimers({ reminderState, onRemindersChange, onMessage }) {
  ...
  function notify(timer) {
    setTimers((current) => current.map((item) => item.id === timer.id ? { ...item, done: true } : item)); timeoutIds.current.delete(timer.id); onMessage?.(`${timer.appliance}-timern är klar.`)
    if (typeof Notification !== 'undefined' && Notification.permission === 'granted') new Notification(`${timer.appliance} – timer klar`, { body: `Dags att kontrollera ${timer.appliance.toLowerCase()}.`, tag: `kitchen-timer-${timer.id}` })
  }
  ...
  async function startTimer(amount, unit) {
    const durationMs = unit === 'seconds' ? amount * 1000 : amount * 60000; const now = Date.now(); const timer = { appliance: selectedAppliance, done: false, durationMs, endsAt: now + durationMs, id: `kitchen-${selectedAppliance.toLowerCase()}-${now}`, unit }
    setTimers((current) => [...current, timer]); const timeoutId = window.setTimeout(() => notify(timer), durationMs); timeoutIds.current.set(timer.id, timeoutId)
    if (unit === 'minutes') { const pushResult = await scheduleBackgroundPush(timer); if (pushResult.error) { onMessage?.(`${selectedAppliance}: ${amount} min startad. Bakgrundsnotisen kunde inte aktiveras: ${pushResult.error.message}`); return } onMessage?.(`${selectedAppliance}: ${amount} min startad med bakgrundsnotis.`); return }
  ...
  async function snooze(timer, minutes = 5) {
    const previousTimeout = timeoutIds.current.get(timer.id); if (previousTimeout) window.clearTimeout(previousTimeout); const next = { ...timer, done: false, endsAt: Date.now() + minutes * 60000, unit: 'minutes' }
    setTimers((current) => current.map((item) => item.id === timer.id ? next : item)); const timeoutId = window.setTimeout(() => notify(next), minutes * 60000); timeoutIds.current.set(timer.id, timeoutId); const pushResult = await scheduleBackgroundPush(next)
    if (pushResult.error) { onMessage?.(`${timer.appliance} snoozad ${minutes} min lokalt. Bakgrundsnotisen kunde inte uppdateras.`); return } onMessage?.(`${timer.appliance} snoozad ${minutes} min med bakgrundsnotis.`)
  }
  ...
  function runWakeSequence(alarm) {
    clearWakeSequence(); const gentleSteps = [{ delay: 0, text: 'God morgon. Det är dags att vakna.', volume: 0.2 }, { delay: 12000, text: 'God morgon. Försök vakna nu.', volume: 0.35 }, { delay: 24000, text: 'Det är dags att gå upp.', volume: 0.55 }, { delay: 36000, text: 'Vakna nu. Ditt väckningslarm har gått.', volume: 0.8 }]; const normalSteps = [{ delay: 0, text: 'God morgon. Det är dags att vakna.', volume: 0.55 }, { delay: 12000, text: 'Vakna nu. Det är dags att gå upp.', volume: 0.85 }]; const steps = alarm.mode === 'gentle' ? gentleSteps : normalSteps
    steps.forEach((step) => { const timeoutId = window.setTimeout(() => { speakWake(step.text, step.volume); if (step === steps[steps.length - 1] && typeof Notification !== 'undefined' && Notification.permission === 'granted') new Notification('Väckarklocka', { body: 'Dags att vakna.', tag: `wake-${alarm.id}` }) }, step.delay); wakeSequenceTimeouts.current.push(timeoutId) })
  }
