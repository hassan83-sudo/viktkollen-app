...
function NoticeKitchenTimers({ reminderState, onRemindersChange, onMessage }) {
  ...
  async function scheduleBackgroundPush(timer) {
    ...
    return upsertKitchenTimerPushSchedule({ appliance: timer.appliance, endsAt: timer.endsAt, timerId: timer.id })
  }

  async function startTimer(amount, unit) {
    const durationMs = unit === 'seconds' ? amount * 1000 : amount * 60000
    const now = Date.now()
    const timer = { appliance: selectedAppliance, done: false, durationMs, endsAt: now + durationMs, id: `kitchen-${selectedAppliance.toLowerCase()}-${now}`, unit }
    ...
    if (unit === 'minutes') {
      ...
      if (pushResult.error) { onMessage?.(`${selectedAppliance}: ${amount} min startad. Bakgrundsnotisen kunde inte aktiveras: ${pushResult.error.message}`); return }
  ...
  async function snooze(timer, minutes = 5) {
    ...
    if (pushResult.error) { onMessage?.(`${timer.appliance} snoozad ${minutes} min lokalt. Bakgrundsnotisen kunde inte uppdateras.`); return }
  ...
  function runWakeSequence(alarm) {
    ...
    steps.forEach((step) => {
      const timeoutId = window.setTimeout(() => {
        ...
        if (step === steps[steps.length - 1] && typeof Notification !== 'undefined' && Notification.permission === 'granted') new Notification('Väckarklocka', { body: 'Dags att vakna.', tag: `wake-${alarm.id}` })
      }, step.delay)
  ...
  async function activateWakeAlarm() {
    ...
    if (pushResult.error) { onMessage?.(`Väckarklocka ${alarmTime} är satt i appen. Bakgrundsnotisen kunde inte aktiveras.`); return }
  ...
  return (
    <section className="notice-card" aria-labelledby="room-timers-heading">
      ...
      <div className="notice-suggestions" aria-label="Rum">
        {rooms.map((room) => <button key={room.id} type="button" aria-expanded={openRoom === room.id} aria-pressed={openRoom === room.id} onClick={() => toggleRoom(room.id)}>{room.label}{room.id === 'kitchen' && activeTimers.length ? ` · ${activeTimers.length} aktiv${activeTimers.length === 1 ? '' : 'a'}` : ''}{room.id === 'bedroom' && wakeAlarm ? ' · larm satt' : ''}</button>)}
      </div>

      {openRoom === 'kitchen' && <div className="notice-room-panel" aria-labelledby="kitchen-timers-heading">
        <div className="notice-actions"><h3 id="kitchen-timers-heading">Kök – snabbtimer</h3><button type="button" onClick={() => setOpenRoom('')}>Stäng</button></div>
        <p>Välj sak och starta direkt. Micro och övriga kan använda både sekunder och minuter.</p>
        <div className="notice-suggestions" aria-label="Kökssaker">{appliances.map((appliance) => <button key={appliance} type="button" aria-pressed={selectedAppliance === appliance} onClick={() => setSelectedAppliance(appliance)}>{appliance}</button>)}</div>
        <h3>{selectedAppliance} · sekunder</h3><div className="notice-suggestions">{secondOptions.map((seconds) => <button key={`sec-${seconds}`} type="button" onClick={() => startTimer(seconds, 'seconds')}>{seconds} sek</button>)}</div>
        <h3>{selectedAppliance} · minuter</h3><div className="notice-suggestions">{minuteOptions.map((minutes) => <button key={`min-${minutes}`} type="button" onClick={() => startTimer(minutes, 'minutes')}>{minutes} min</button>)}</div>
        {activeTimers.length > 0 && <div className="notice-kitchen-active"><h3>Aktiva timers</h3><ul className="notice-reminder-list">{activeTimers.map((timer) => <li key={timer.id}><strong>{timer.appliance}</strong><span>{formatRemaining(timer.endsAt - Date.now())} kvar</span><div className="notice-actions"><button type="button" onClick={() => snooze(timer, 5)}>Snooze 5 min</button><button type="button" onClick={() => complete(timer)}>Klar</button></div></li>)}</ul></div>}
      ...
      {openRoom === 'bedroom' && <div className="notice-room-panel" aria-labelledby="bedroom-heading">
        ...
        <div className="notice-form-grid"><label>Väckningstid<input type="time" value={alarmTime} onChange={(event) => setAlarmTime(event.target.value)} /></label><label>Väckning<select value={wakeMode} onChange={(event) => setWakeMode(event.target.value)}><option value="gentle">Mjuk: låg röst → högre → larm</option><option value="normal">Normal: röst → högre röst</option></select></label></div>
        <div className="notice-actions"><button className="primary-button" type="button" onClick={activateWakeAlarm}>Sätt väckarklocka</button>{wakeAlarm && <button type="button" onClick={() => snoozeWake(5)}>Snooze 5 min</button>}{wakeAlarm && <button type="button" onClick={stopWakeAlarm}>Stäng av</button>}</div>
