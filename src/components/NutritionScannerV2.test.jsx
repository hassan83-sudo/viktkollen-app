...
describe('NutritionScannerV2', () => {
  ...
  it('keeps local analysis independent from the remote provider chunk', () => {
    ...
    expect(source).toContain("t('scanner.localNotAiInterpreted')")
    expect(source).toContain('scanner-review-hero')
    expect(source).toContain("t('scanner.analyzing')")
    expect(source).toContain('scanner.stages.identifying')
    expect(source).toContain('scanner.stages.portions')
    expect(source).toContain('scanner.stages.nutrition')
    expect(source).toContain("t('scanner.identifiedComponents')")
  ...
  it('keeps scanner review labels Swedish without changing internal enums', () => {
    ...
    expect(source).toContain("high: 'scanner.confidence.high'")
    expect(source).toContain("medium: 'scanner.confidence.medium'")
    expect(source).toContain("low: 'scanner.confidence.low'")
    expect(source).toContain("usable: 'scanner.imageQuality.usable'")
    expect(source).toContain("poor: 'scanner.imageQuality.poor'")
    expect(source).toContain("good: 'scanner.imageQuality.good'")
    expect(source).toContain("fried: 'scanner.cookingMethods.fried'")
    expect(source).toContain("raw: 'scanner.cookingMethods.raw'")
    expect(source).toContain("fat: 'scanner.categories.fat'")
    expect(source).toContain('confidenceLabel(analysis.confidence.level, t)')
    expect(source).toContain('imageQualityLabel(analysis.imageQuality, t)')
    expect(source).toContain('component.cookingMethods.map((method) => cookingMethodLabel(method, t))')
  ...
  it('presents alternatives as variant uncertainty instead of contradicting confidence', () => {
    ...
    expect(source).toContain('scanner.component.summaryNeedsReview')
    expect(source).toContain('scanner.couldAlsoBe')
    ...
    expect(source).toContain('scanner.component.summaryPortionEstimated')
  ...
  it('shows database status buckets including AI estimates in review text', () => {
    ...
    expect(source).toContain("t('scanner.dbMatchSummary'")
    expect(source).toContain('aiEstimate: ingredientMatchStatusCounts.aiEstimate')
    expect(source).toContain('total: ingredientMatchStatusCounts.total')
  })
  ...
  it('keeps ingredient editing fields and database provenance available in compact cards', () => {
    ...
    expect(editorBody).toContain("t('scanner.ingredient.name')")
    expect(editorBody).toContain("t('scanner.ingredient.amount')")
    expect(editorBody).toContain("t('scanner.ingredient.unit')")
    ...
    expect(editorBody).toContain("t('scanner.ingredient.uncertain')")
    expect(editorBody).toContain("t('scanner.ingredient.useDatabase'")
    expect(editorBody).toContain('dataSourceLabel(item.dataSource, t)')
    expect(editorBody).toContain('scanner.ingredient.multipleMatches')
    expect(editorBody).toContain("t('common:remove')")
  })
  ...
  it('keeps native image picker controls separate from analysis touch handlers', () => {
    ...
    expect(consentStatusBlock).toContain("t('scanner.consentGranted')")
    expect(consentStatusBlock).toContain("t('scanner.revokeConsent')")
  })
  ...
  it('rejects non-remote results from the remote analysis button', () => {
    ...
    expect(analyzeBody).toContain("t('scanner.errors.remoteInvalid')")
  })
