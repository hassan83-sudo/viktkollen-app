...
describe('OverviewDashboard', () => {
  ...
  it('uses text fallbacks instead of fake zero values when data is missing', () => {
    ...
    expect(markup).not.toContain('Vänner')
  })

  it('shows social empty state only when the feature is on', () => {
    const markup = renderOverview({
      featureFlags: { social: true },
      isAuthenticated: true,
    })
    expect(markup).toContain('Vänner')
    expect(markup).toContain('Träna och håll kontakten tillsammans.')
