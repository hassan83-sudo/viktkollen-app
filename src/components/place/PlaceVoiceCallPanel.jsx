...
// A11Y-8Z3 (B10): texts set inside effects use the i18n instance, so the
// audio effect does not restart on t changes.
const callText = (key) => i18n.t(`place:voiceCall.${key}`)
...
function PlaceVoiceCallPanel({ familyId, targetUserId, familyMembers, open, onClose }) {
  ...
  async function refresh(){const r=await loadActivePlaceVoiceCalls();setUserId(r.userId||null);setCalls(r.data||[]);if(r.error)setNotice(r.error.message||callText('loadFailed'))}
  ...
  useEffect(()=>{if(!accepted||!relevant?.id||!userId){if(activeAudioCallRef.current){stopPlaceVoiceAudio(activeAudioCallRef.current);activeAudioCallRef.current=null;setAudioState('')}return}if(activeAudioCallRef.current===relevant.id)return;let cancelled=false;activeAudioCallRef.current=relevant.id;setAudioState(callText('audioConnecting'));startPlaceVoiceAudio({callId:relevant.id,initiator:relevant.caller_user_id===userId,onRemoteStream:stream=>{if(cancelled)return;const el=audioRef.current;if(el){el.srcObject=stream;el.play().catch(()=>setAudioState(callText('audioTapToStart')))}},onState:state=>{if(!cancelled)setAudioState(state==='connected'?callText('audioConnected'):state==='failed'?callText('audioFailed'):callText('audioConnecting'))}}).then(()=>{if(walkie)setPlaceVoiceMicrophone(relevant.id,false)}).catch(error=>{if(!cancelled){setAudioState('');setNotice(error?.name==='NotAllowedError'?callText('micBlocked'):error?.message||callText('micFailed'))}});return()=>{cancelled=true}},[accepted,relevant?.id,relevant?.caller_user_id,userId])
  ...
  async function start(){setBusy(true);setNotice('');const r=await startPlaceVoiceCall({familyId,calleeUserId:targetUserId});if(r.error)setNotice(r.error.message||t('voiceCall.startFailed'));else{setCalls(c=>[r.data,...c]);setNotice(t('voiceCall.calling',{name:targetName}))}setBusy(false)}
  async function update(status){if(!relevant)return;setBusy(true);if(status==='ended'||status==='declined'){stopPlaceVoiceAudio(relevant.id);activeAudioCallRef.current=null}const r=await updatePlaceVoiceCall(relevant.id,status);if(r.error)setNotice(r.error.message||t('voiceCall.updateFailed'));else await refresh();setBusy(false)}
  ...
  if(!open)return null
  return <section className="family-map-history" aria-label={t('voiceCall.aria', { name: targetName })}><audio ref={audioRef} autoPlay playsInline/><div className="family-map-history-heading"><strong>📞 {t('voiceCall.title',{name:targetName})}</strong><button type="button" onClick={onClose}>{t('common:actions.close')}</button></div>{!relevant?<><p>{t('voiceCall.intro')}</p><button type="button" onClick={start} disabled={busy||targetUserId===userId}>{t('voiceCall.call')}</button></>:null}{outgoing?<><p>📞 {t('voiceCall.calling',{name:targetName})}</p><button type="button" onClick={()=>update('ended')} disabled={busy}>{t('common:actions.cancel')}</button></>:null}{incoming?<><p>📞 {t('voiceCall.incoming',{name:targetName})}</p><div className="family-map-person-actions"><button type="button" onClick={()=>update('accepted')} disabled={busy}>{t('voiceCall.answer')}</button><button type="button" onClick={()=>update('declined')} disabled={busy}>{t('voiceCall.decline')}</button></div></>:null}{accepted?<><p>🟢 {t('voiceCall.inProgress',{name:targetName})}</p><small role="status">{audioState||t('voiceCall.micPending')}</small><label className="family-map-frequency"><span>{t('voiceCall.walkieLabel')}</span><input type="checkbox" checked={walkie} onChange={e=>setWalkie(e.target.checked)}/></label>{walkie?<div className="family-map-walkie"><p className="family-map-walkie-status" role="status">{talking?t('walkie.statusTalking'):t('walkie.statusReady')}</p><button type="button" className="family-map-walkie-toggle" onClick={()=>talk(!talking)}>{talking?t('walkie.stop'):t('walkie.start')}</button><small>{t('walkie.toggleHint')}</small><button type="button" aria-pressed={talking} onClick={holdButtonClick} onPointerDown={startHold} onPointerUp={endHold} onPointerCancel={endHold} onPointerLeave={endHold} style={{touchAction:'none'}}><span aria-hidden="true">🎙️ </span>{talking?t('walkie.holdActive'):t('walkie.hold')}</button><small>{t('walkie.holdHint')}</small></div>:null}<br/><button type="button" onClick={()=>update('ended')} disabled={busy}>{t('voiceCall.end')}</button></>:null}{notice?<p className="family-map-card-notice" role="status">{notice}</p>:null}</section>
}
