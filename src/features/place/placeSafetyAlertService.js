...
function unavailable(message = 'Trygghetslarm kräver att du är inloggad.') {
...
export async function loadSafetyAlerts() {
  if (!supabase) return { data: [], error: new Error('Trygghetslarm är inte anslutet.') }
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('id,family_id,sender_user_id,reason,latitude,longitude,accuracy_meters,created_at')
...
export async function sendSafetyAlert(reason) {
  if (!allowedReasons.has(reason)) return unavailable('Ogiltig anledning för trygghetslarm.')
  if (!supabase) return unavailable('Trygghetslarm är inte anslutet.')
...
export function subscribeSafetyAlerts(onAlert) {
  if (!supabase || typeof onAlert !== 'function') return () => {}

  const channel = supabase
    .channel(`place-safety-alerts-${Math.random().toString(36).slice(2)}`)
    .on(
      'postgres_changes',
      { event: 'INSERT', schema: 'public', table: 'place_safety_alerts' },
      (payload) => {
        if (payload?.new) onAlert(payload.new)
      },
    )
    .subscribe()
