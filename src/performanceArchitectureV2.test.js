...
describe('Performance Architecture V2 contracts', () => {
  it('keeps cloud and sync implementations out of App static imports', () => {
    ...
    expect(source).toContain("const AchievementCenter = lazy(() => import('./components/AchievementCenter.jsx'))")
    expect(source).toContain("const SocialCenter = lazy(() => import('./components/SocialCenter.jsx'))")
    expect(source).toContain("const DataImportCenter = lazy(() => import('./components/DataImportCenter.jsx'))")
    expect(source).toContain("const DataExportCenter = lazy(() => import('./components/DataExportCenter.jsx'))")
    expect(source).not.toContain("from './services/import/")
    expect(source).not.toContain("from './services/export/")
    expect(source).not.toContain("from './services/achievements/")
    expect(source).not.toContain("from './services/social/")
