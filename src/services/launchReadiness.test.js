...
describe('launchReadiness', () => {
  it('builds a masked development readiness report', () => {
    ...
    expect(report.diagnostics.journeyBuilder).toBe('pass')
    expect(report.diagnostics.journeyTimelineUi).toBe('lazy-loaded')
    expect(report.diagnostics.journeyAiMinimization).toBe('consent-gated-summary-only')
    expect(report.diagnostics.journeyPersistence).toBe('derived-only')
    expect(report.diagnostics.smartHabitGoalEngine).toBe('pass')
    expect(report.diagnostics.smartHabitGoalUi).toBe('lazy-loaded')
    expect(report.diagnostics.smartHabitGoalAiMinimization).toBe('consent-gated-summary-only')
    expect(report.diagnostics.smartHabitGoalPersistence).toBe('existing-goals-habits-state')
