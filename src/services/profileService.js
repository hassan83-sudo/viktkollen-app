...
export function normalizeWeightDirection(value, profile = {}) {
  const text = [value, profile.weightGoal, profile.goalDirection, profile.goal, profile.target].join(' ').toLocaleLowerCase('sv-SE')
...
function normalizeOptionalWeight(value) {
  ...
  return parsed === null || parsed < 25 || parsed > 350 ? null : Number(parsed.toFixed(1))
}
...
export function normalizeProfile(rawProfile = {}, options = {}) {
  ...
  const weightDirection = normalizeWeightDirection(source.weightDirection, { ...source, goalWeight, startWeight })
  ...
  return {
    ...
    provenance: {
      ...
      dietaryPreferences: provenanceFor(dietaryPreferences.dietType !== 'omnivore' || dietaryPreferences.avoidedFoods.length || dietaryPreferences.preferredFoods.length || Object.values(dietaryPreferences.preferences || {}).some(Boolean) ? 'set' : '', provenance.dietaryPreferences),
    ...
    units: { height: units.height === 'in' ? 'in' : 'cm', weight: units.weight === 'lb' ? 'lb' : 'kg' },
    updatedAt,
    weightDirection,
...
export function validateProfileDraft(draft = {}) {
  ...
  if (heightText && normalizeHeightCm(heightText) === null) errors.height = 'Längd behöver anges i ett rimligt cm-värde.'
  if (startWeightText && normalizeOptionalWeight(startWeightText) === null) errors.startWeight = 'Startvikt behöver vara ett rimligt kg-värde.'
  if (goalWeightText && normalizeOptionalWeight(goalWeightText) === null) errors.goalWeight = 'Målvikt behöver vara ett rimligt kg-värde.'
  if (draft.activityLevel && !normalizeActivityLevel(draft.activityLevel)) errors.activityLevel = 'Välj en giltig aktivitetsnivå.'
  return errors
}
...
export function profileDraftToProfile(draft = {}, options = {}) {
  ...
  return {
    errors: {},
    profile: normalizeProfile({ ...draft, dietaryPreferences: { avoidedFoods: draft.avoidances, dietType: draft.dietaryPattern } }, { markCompleted: true, now: options.now }),
  }
}

export function hasUsableProfile(profile = {}) {
  // A weight/personalization profile is optional. Missing profile data must never
  // block app start; features that need it can ask for it in context instead.
  if (!isObject(profile)) return true
  const normalized = normalizeProfile(profile)
  return normalized.onboardingCompleted || Boolean(normalized.displayName || normalized.heightCm || normalized.startWeight || normalized.goalWeight || normalized.activityLevel)
}
