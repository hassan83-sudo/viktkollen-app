-- BILL-6C5A apply a stored next-period plan during period advance.
...
alter table billing.subscription_events
  ...
  add column if not exists from_plan_version integer,
  add column if not exists to_plan_version integer;
...
begin
  ...
  next_plan_id := existing.plan_id;
  next_plan_version := existing.plan_version;
  if existing.pending_plan_change = 'next_period' then
    if existing.pending_plan_id is null or existing.pending_plan_id = 'plan.free' then
      raise exception 'invalid_pending_plan';
    end if;
    select p.version into next_plan_version
    from billing.plans p
    where p.plan_id = existing.pending_plan_id
      and p.active = true;
    if next_plan_version is null then
      raise exception 'invalid_pending_plan';
    end if;
    next_plan_id := existing.pending_plan_id;
  end if;
  ...
  set
    ...
    pending_plan_change = case
      when existing.pending_plan_change = 'next_period' then null
      else s.pending_plan_change
    end,
    pending_plan_id = case
      when existing.pending_plan_change = 'next_period' then null
      else s.pending_plan_id
    end,
