...
test.describe('release PWA contract', () => {
  ...
  test('does not preload lazy release-gate or heavy feature chunks', async ({ page }) => {
    ...
    expect(preloadHrefs.join('\n')).not.toMatch(/AdaptiveCoachPanel|CoachMemoryReview|CoachPlanCenter|NutritionCoachCenter|PredictionCenter|coachContextSelector|ReminderCenter|LaunchReadinessPanel|ManualAcceptanceRunner|CloudBackupPanel|DataImportCenter|DataExportCenter|ReportDrilldown|NotificationCenter|InsightsCenter|AchievementCenter|SocialCenter/)
  })
})
